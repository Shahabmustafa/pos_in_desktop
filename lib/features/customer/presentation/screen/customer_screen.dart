...
import '../widget/customer_form_dialog.dart';

/// Customer module: list + add / edit / delete.
class CustomerScreen extends StatefulWidget {
  const CustomerScreen({super.key, this.provider});
...
class _CustomerScreenState extends State<CustomerScreen> {
  late final CustomerProvider _provider = widget.provider ?? CustomerProvider();
  ...
  Future<void> _openForm([CustomerModel? existing]) async {
    final result = await showDialog<CustomerModel>(
      context: context,
      builder: (_) => CustomerFormDialog(initial: existing),
    );
    ...
    if (ok) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Customer "${result.name}" saved')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final canAdd = access.can('customer', PermAction.add);
    final canEdit = access.can('customer', PermAction.edit);
    final canDelete = access.can('customer', PermAction.delete);
                  ...
                  child: SectionCard(
                    ...
                    child: _provider.items.isEmpty
                        ? EmptyState(
                            ...
                            actionLabel:
                                canAdd ? 'Add your first customer' : null,
                                          ...
                                          IconButton(
                                            ...
                                            onPressed: () => Navigator.of(context)
                                              ...
                                              builder: (_) => PartyLedgerScreen(
                                                initialKind: LedgerKind.customer,
                                                initialPartyId: c.id,
                                              ),
                                            )),
                                          ),
